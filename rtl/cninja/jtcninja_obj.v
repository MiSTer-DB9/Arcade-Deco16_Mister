/*  See jtcninja_game.v header.
    ...
    Sprite engine for Data East cninja.cpp (Joe & Mac) - the DECO decospr /
    MXC-06 family. Adapted from cores/cop/hdl/jtcop_obj_draw.v with the cninja
    decospr entry format (see doc/sprites.md, verified vs MAME scene 1500):

      word0 (y) : [8:0]ypos [10:9]vsize(1/2/4/8) [11]wide [12]flash
                  [13]hflip [14]vflip [15]extra-pri
      word1     : tile code (masked &~(vsize-1))
      word2 (x) : [8:0]xpos [13:9]colour(5b) [15:14]priority

    Placement: bottom = 256-ypos, top = bottom-16*vtiles ; x = 240-xpos.
    Drawn back-to-front. Pen = 0x300 + colour*16 + pixel (gfx base 768).

    First cut: reads sprite RAM directly (works for scene replay; the buffered_
    spriteram DMA copy is TODO), single 16px column (the `wide` wing is TODO),
    no priority mixing (sprites composite on top).  Tile ROM decode reuses the
    verified tile plane order {d[31],d[23],d[15],d[7]} + the BA3 post_addr remap.
*/
module jtcninja_obj(
    input             rst,
    input             clk,
    ...
    output     [10:0] pxl          // {pri[1:0], colour[4:0], pixel[3:0]}; pixel0 = transparent
);
...
always @(posedge clk, posedge rst) begin
    if( rst ) begin
        ...
        hflip<=0; vflip<=0; id<=0; pal<=0; pri<=0; xpos<=0; veff<=0; cen2<=0;
    end else begin
        HSl <= HS; LVl <= LVBL; draw <= 0; cen2 <= ~cen2;
        if( !LVBL && LVl ) frame <= ~frame;
        // Sprite-vs-sprite priority is HIGH-slot-on-top (verified against MAME:
        // the blue dino, slots 26-42, draws in front of the green neck, slots
        // 10-21). With the last-opaque-wins line buffer that means scanning
        // LOW->HIGH (0..255) so the highest slot is drawn last and ends on top.
...
wire [ 3:0] draw_pxl  = hflip ? { draw_data[24], draw_data[16], draw_data[8], draw_data[0] } :
                                { draw_data[31], draw_data[23], draw_data[15], draw_data[7] };
wire [10:0] buf_wdata = { pri, pal, draw_pxl };
...
always @(posedge clk, posedge rst) begin
    if( rst ) begin
        ...
        if( !buf_we && rom_cs && fresh && rom_good && rom_ok && draw_cnt==0 ) begin
            ...
            // PREFETCH: as soon as the 1st half (half==1) is captured, issue the
            // 2nd half right away so its SDRAM latency overlaps the 8px shift
            // below instead of stalling after it (~2x sprite throughput on real
            // SDRAM). 2nd half captured (half==0) -> no further fetch this column.
        ...
        if( buf_we ) begin
            ...
            if( draw_cnt==0 ) begin
                buf_we <= 0;
                if( half ) begin                                 // 1st half shifted; the
                    half <= 0; draw_cnt <= 0;                    // 2nd half is already
                    // fetching (prefetched at capture) -> capture grabs it next.
...
jtframe_obj_buffer #(.DW(11), .ALPHA(11'd0)) u_buffer(
    .clk     ( clk        ),
    .LHBL    ( LHBL       ),
    .flip    ( 1'b0       ),
    .wr_data ( buf_wdata  ),
